...
export function UserPermissionPreviewDialog({ tenantId, data, onClose }: { tenantId: string; data: OverviewSnapshot; onClose: () => void }) {
  ...
  const resolvedAccessGroups = result?.access_group_ids.map((accessGroupId) => ({
    accessGroupId,
    displayName: data.accessGroups.groups.find((group) => group.access_group_id === accessGroupId)?.display_name ?? accessGroupId,
  })) ?? []
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) { generation.current++; onClose() } }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl" data-testid="user-permission-preview"><DialogHeader><DialogTitle>{t("View as user permissions")}</DialogTitle><DialogDescription>{t("Preview permissions only. Conversations, memory, attachments and personal credentials are not loaded. No models or tools are invoked.")}</DialogDescription></DialogHeader>
    ...
    {result ? <div className="space-y-5"><Alert className="sticky top-0 z-10 bg-background"><AlertDescription className="flex flex-wrap items-start gap-3"><div className="min-w-0 flex-1 space-y-1"><div>{t("Viewing permissions as {{name}}", { name: result.subject_display_name })} · {t(result.role)} · {result.runtime_id} · {result.client_id}</div><div className="flex flex-wrap items-center gap-2"><span className="font-medium text-foreground">{t("Resolved access groups")}</span>{resolvedAccessGroups.length ? resolvedAccessGroups.map((group) => <Badge key={group.accessGroupId} title={group.accessGroupId} variant="outline">{group.displayName}</Badge>) : <span>{t("None")}</span>}</div><p className="text-xs">{t("These groups are resolved by the server for this preview and are not supplied by the request.")}</p></div><Button size="sm" variant="outline" onClick={() => { generation.current++; onClose() }}>{t("Exit preview")}</Button></AlertDescription></Alert>
