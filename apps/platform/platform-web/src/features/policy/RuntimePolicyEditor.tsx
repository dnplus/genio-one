...
function emptyScope(): RuntimePolicyScope {
  return { access_group_ids: [], subject_ids: [], organization_ids: [], roles: [], client_ids: [], bot_ids: [], runtime_ids: [] }
}

function scopeHasSelection(scope: RuntimePolicyScope): boolean {
  return Object.values(scope).some((values) => Array.isArray(values) && values.length > 0)
}
...
function cloneDefinition(value: RuntimePolicyDefinition): RuntimePolicyDefinition {
  return {
    ...
    scope: {
      access_group_ids: [...(value.scope.access_group_ids ?? [])],
...
export function finalRuntimePolicyDefinition(value: RuntimePolicyDefinition): RuntimePolicyDefinition {
  ...
  return {
    ...
    scope: {
      access_group_ids: [...new Set(value.scope.access_group_ids ?? [])].sort(),
...
export function RuntimePolicyEditor({
  ...
  const accessGroupOptions = useMemo<SearchableSelectOption[]>(() => data.accessGroups.groups
    .filter((group) => group.enabled)
    .map((group) => ({ value: group.access_group_id, label: group.display_name, description: group.description || group.access_group_id }))
    .sort((left, right) => left.label.localeCompare(right.label)), [data.accessGroups.groups])
  ...
  const runtimeOptions = useMemo<SearchableSelectOption[]>(() => {
    return [{ value: "codex", label: t("Codex"), description: t("Personal Codex runtime"), searchText: "codex" }]
  }, [t])
  const capabilityOptions = useMemo<SearchableSelectOption[]>(() => {
    return RUNTIME_CAPABILITY_IDS.map((capabilityId) => ({
      value: capabilityId,
      label: capabilityId,
      description: RUNTIME_CAPABILITY_REGISTRY[capabilityId].join(", "),
      searchText: capabilityId,
    }))
  }, [])
  ...
  function updateScope<Key extends keyof RuntimePolicyScope>(key: Key, value: RuntimePolicyScope[Key]) {
    ...
    setDefinition((current) => ({ ...current, scope: { ...current.scope, [key]: value }, rules: key === "runtime_ids" && current.scope.runtime_ids.length === 1 && Array.isArray(value) && value.length === 1
  ...
  function checkDefinition() {
    ...
    value.rules.forEach((rule) => {
      ...
      const registeredActions = runtimeCapabilityActions(rule.target.capability_id)
  ...
  async function validate() {
    if (!draft || draft.content.kind !== "RUNTIME_CAPABILITY" || dirty || draft.lifecycle !== "DRAFT" || !checkDefinition()) return
    setBusy(true)
    setError("")
    try {
      const updated = await validateRuntimePolicyDraft(tenantId, policyId, draft.version, draft.content_digest)
      setDraft(updated)
      setNotice("RUNTIME_POLICY_DRAFT_VALIDATED")
      await onDraftSaved?.()
    } catch (caught) { setError(caught instanceof Error ? caught.message : "RUNTIME_POLICY_VALIDATE_FAILED") }
    finally { setBusy(false) }
  }

  async function review() {
    if (!draft || draft.content.kind !== "RUNTIME_CAPABILITY" || dirty || draft.lifecycle !== "VALIDATED") return
    setBusy(true)
    setError("")
    try {
      const updated = await reviewRuntimePolicyDraft(tenantId, policyId, draft.version, draft.content_digest)
      setDraft(updated)
      setNotice("RUNTIME_POLICY_DRAFT_REVIEWED")
      await onDraftSaved?.()
    } catch (caught) { setError(caught instanceof Error ? caught.message : "RUNTIME_POLICY_REVIEW_FAILED") }
    finally { setBusy(false) }
  }

  async function publish() {
    if (!draft || draft.content.kind !== "RUNTIME_CAPABILITY" || dirty || draft.lifecycle !== "REVIEWED") return
    setBusy(true)
    setError("")
    try {
      await publishRuntimePolicyDraft(tenantId, policyId, draft.version, draft.content_digest)
      setDraft(null)
      setEditing(false)
  ...
  const scopeFields: Array<{ key: keyof RuntimePolicyScope; label: string; options: SearchableSelectOption[]; placeholder: string; searchPlaceholder: string }> = [
    { key: "access_group_ids", label: "Access groups", options: accessGroupOptions, placeholder: "Choose an Access Group", searchPlaceholder: "Search" },
