...
test("PostgreSQL runtime policy revisions are durable and immutable", { skip: process.env.GENIO_ONE_RUNTIME_POLICY_PERSISTENCE_TEST !== "1" }, async () => {
  ...
  try {
    await sql.query(await readFile(new URL("../migrations/094_one_policy_runtime_catalog.sql", import.meta.url), "utf8"))
    const store = createPostgresRuntimePolicyStore({ sql, now: () => 1_757_000_000 })
    ...
    const disabled = await store.setEnabled({ tenantId: "tenant-qa", policyId: "one-policy.runtime.capabilities", expectedRevision: 2, enabled: false, publishedBy: "person-admin" })
    ...
    const concurrent = await Promise.allSettled([
      ...
      store.setEnabled({ tenantId: "tenant-qa", policyId: "one-policy.runtime.capabilities", expectedRevision: 3, enabled: true, publishedBy: "person-admin" }),
    ])
...
test("PostgreSQL runtime draft publication rolls back cleanup failures and serializes concurrent editors", { skip: process.env.GENIO_ONE_RUNTIME_POLICY_PERSISTENCE_TEST !== "1" }, async () => {
  ...
  try {
    for (const file of ["084_first_party_policy_seeds.sql", "085_policy_authoring_drafts.sql", "090_policy_draft_monotonic_versions.sql", "094_one_policy_runtime_catalog.sql"]) {
      await sql.query(await readFile(new URL(`../migrations/${file}`, import.meta.url), "utf8"))
    }
    ...
    const request = { tenantId, policyId, expectedVersion: draft.version, publishedBy: "admin-qa" }
    ...
    assert.deepEqual(await drafts.get(tenantId, key), draft)
    ...
    const concurrent = await Promise.allSettled([
      ...
      drafts.save(tenantId, key, { expected_version: draft.version, base_revision: 0, content: { kind: "RUNTIME_CAPABILITY", definition: { ...definition, display_name: "Concurrent edit" } } }),
    ])
    assert.equal(concurrent.filter((result) => result.status === "fulfilled").length, 1)
    ...
    if (remaining) {
      ...
      await store.publishDraft({ ...request, expectedVersion: remaining.version })
    }
