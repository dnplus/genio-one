...
test("runtime draft publication consumes the exact version and rejects concurrent retries", async () => {
  ...
  const draft = await drafts.save("tenant", key, { expected_version: 0, base_revision: 0, content: { kind: "RUNTIME_CAPABILITY", definition } })
  const validated = await drafts.validate("tenant", key, { expectedVersion: draft.version, expectedContentDigest: draft.content_digest })
  const reviewed = await drafts.review("tenant", key, { expectedVersion: validated.version, expectedContentDigest: validated.content_digest })
  const request = { tenantId: "tenant", policyId: "policy", expectedVersion: reviewed.version, expectedContentDigest: reviewed.content_digest, publishedBy: "admin" }
...
test("runtime publication failure retains the draft and the previous revision", async () => {
  ...
  const validated = await drafts.validate("tenant", key, { expectedVersion: draft.version, expectedContentDigest: draft.content_digest })
  const reviewed = await drafts.review("tenant", key, { expectedVersion: validated.version, expectedContentDigest: validated.content_digest })
  await assert.rejects(store.publishDraft({ tenantId: "tenant", policyId: "policy", expectedVersion: reviewed.version, expectedContentDigest: reviewed.content_digest, publishedBy: "admin" }), { code: "POLICY_REVISION_CONFLICT" })
  assert.equal((await store.list("tenant")).length, 0)
  assert.deepEqual(await drafts.get("tenant", key), reviewed)
})
